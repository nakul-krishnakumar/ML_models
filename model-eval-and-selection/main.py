...
# custom utilities from utils.py
import utils
...
data = np.loadtxt("model-eval-and-selection/dataset.csv", delimiter=',')
...
# MSE is computed over the training set and not over the entire input set
...
"""
    have to use the mean and standard deviation of the training set when
    scaling the cross validation set. This is to ensure that your input 
    features are transformed as expected by the model. One way to gain 
    intuition is with this scenario:
...
data = np.loadtxt('model-eval-and-selection/data_w3_ex2.csv', delimiter=',')
